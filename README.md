...
- 📋 **Настраиваемые правила** - подключаемые промпты из Markdown файлов для кастомизации поведения AI
...
Вы можете настроить поведение AI-ассистента с помощью Markdown файлов с правилами, которые автоматически добавляются к системному промпту.

#### Создание правил

1. **Глобальные правила** (применяются ко всем проектам):
   ```
   ~/.ride/rules/
   ├── coding-style.md
...
2. **Проектные правила** (применяются только к текущему проекту):
   ```
   <PROJECT_ROOT>/.ride/rules/
   ├── architecture-patterns.md
   ├── naming-conventions.md
   └── specific-guidelines.md
   ```

#### Настройка через UI

1. Откройте **Settings → Tools → Ride → Rules**
2. Включите опцию **"Включить настраиваемые правила"**
3. Используйте кнопки для открытия директорий правил
4. Создайте шаблоны правил с помощью кнопок **"Create Global Template"** и **"Create Project Template"**
5. Просмотрите активные правила в секции **"Rules Preview"**

#### Пример файла правил

```markdown
# Стиль кодирования Kotlin

Всегда следуй этим правилам при написании Kotlin кода:

- Используй descriptive имена переменных
- Применяй camelCase для локальных переменных
...
1. **Проектные правила** имеют высший приоритет
2. **Глобальные правила** применяются после проектных
3. Правила конкатенируются в алфавитном порядке файлов
4. Максимальный размер всех правил: 50,000 символов
