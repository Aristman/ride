...
import ru.marslab.ide.ride.agent.testing.DartTestingAgent
import ru.marslab.ide.ride.agent.testing.JavaTestingAgent
import ru.marslab.ide.ride.agent.testing.KotlinTestingAgent
...
class TestingAgentOrchestrator(
    ...
    private val detectors: List<LanguageTestingAgent> = listOf(
        KotlinTestingAgent(), JavaTestingAgent(),
        DartTestingAgent()
    ),
    private val frameworkDetector: TestFrameworkDetector = JvmTestFrameworkDetector(A2AProjectStructureProvider())
) {
    private val logger = Logger.getInstance(TestingAgentOrchestrator::class.java)
