...
data class ToolAgentStatusMessage(
    ...
    fun toHtml(): String {
        ...
        return """
            ...
            <script>
                (function() {
                    // Убедем существующую функцию если она есть
                    if (window.toggleOutput) {
                        console.log('toggleOutput function already exists');
                    } else {
                        console.log('Creating new toggleOutput function');
                    ...
                    // Добавляем обработчик кликов на все заголовки результатов
                    document.addEventListener('click', function(event) {
                        ...
                        if (outputHeader) {
                            ...
                            const outputId = outputHeader.getAttribute('data-output-id') || outputHeader.parentElement?.id;
                            ...
                            console.log('❌ Click not on output header');
                        }
                    });

                    // Дополнительный обработчик через data-attribute для надежности
                    document.addEventListener('click', function(event) {
                        const element = event.target;
                        const outputId = element.getAttribute('data-output-id');
                        if (outputId) {
                            console.log('🔗 Direct data-output-id click:', outputId);
                            if (window.toggleOutput) {
                                window.toggleOutput(outputId);
                            }
                        }
                    ...
                    console.log('🎯 Tool Agent Status scripts initialized');
                })();
            </script>
        """.trimIndent()
    }
