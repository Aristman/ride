...
/**
 * Реализация LLM провайдера для Hugging Face Router (OpenBuddy Llama3-8B)
 * Совместим с интерфейсом OpenAI Chat Completions
 */
class HuggingFaceOpenbuddyLlama3Provider(
    private val config: HuggingFaceConfig
) : LLMProvider {

    private val logger = Logger.getInstance(HuggingFaceOpenbuddyLlama3Provider::class.java)
    ...
    override suspend fun sendRequest(
        ...
        logger.info(
            "Sending request to Hugging Face OpenBuddy Llama3, systemPrompt length: ${systemPrompt.length}, userMessage length: ${userMessage.length}, conversationHistory size: ${conversationHistory.size}"
        )
        return try {
    ...
    override fun getProviderName(): String = "HuggingFaceOpenbuddyLlama3"
