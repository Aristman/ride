...
- **Мониторинг и метрики**: сбор производительности и статусов.
 - **Тестирование**: модульные/интеграционные/нагрузочные на текущем тестовом стеке проекта (kotlin-test, coroutines-test, mockk).
...
Реализация MVP: in-memory на `kotlinx.coroutines.channels.Channel`, request-response через `withTimeout`. Персистентность шины не входит в MVP и будет реализована позже. Возможность последующей замены транспорта (Ktor WS/HTTP) без изменения интерфейса.
...
- Интеграция с UI: прогресс-ивенты через `ChatService`; отображать время выполнения и затраченные токены (если доступны).
...
- **Фреймворк**: использовать текущий тестовый стек проекта: `org.jetbrains.kotlin:kotlin-test`, `kotlinx-coroutines-test`, `io.mockk:mockk`.
- **Юнит-тесты**:
...
- Написаны тесты (текущий стек проекта), покрытие и стабильность подтверждены.
...
1. Нужны ли кросс-сессионные взаимодействия агентов (между чатами) на старте?
2. Детали метрик агрегирования/экспорта на будущее (сейчас — хранение в памяти, в UI отображать время и токены).
3. Ограничения параллельности по умолчанию (глобальные/на план/на агента)?
