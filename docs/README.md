...
## 🎤 Голосовой ввод (STT) через Yandex SpeechKit

Этот плагин поддерживает голосовой ввод: запись с микрофона и распознавание речи через Yandex SpeechKit (STT v2). Распознанный текст автоматически подставляется в поле ввода чата без авто-отправки.

### Требования
- Доступ к микрофону в ОС.
- Действительные креденшлы Yandex Cloud:
  - `YANDEX_API_KEY`
  - `YANDEX_FOLDER_ID`

### Настройка
1. Откройте настройки плагина Ride и укажите:
   - API ключ: сохранится в PasswordSafe (`PluginSettings.getApiKey()`)
   - Folder ID: `PluginSettings.folderId`
2. Язык по умолчанию: `ru-RU` (можно переопределить в коде через `SttConfig`).

### Использование в чате
- Кнопка «🎤» расположена внизу, справа под полем ввода.
- Нажмите «🎤» — начнётся запись (появится системное сообщение «Запись…»), иконка сменится на «⏹».
- Нажмите «⏹» — начнётся распознавание, после чего текст подставится в поле ввода.

### Реализация (для разработчиков)
- Domain: `ru.marslab.ide.ride.stt.domain` (`SpeechToTextService`, `SttConfig`).
- Infrastructure: `YandexSpeechSttService` (HTTP, JSON), `AudioRecorder` (LPCM 16 kHz mono).
- Application: `SttFacade` — фасад записи/распознавания.
- UI: кнопка в `ChatUiBuilder`, логика в `ChatPanel` (старт/стоп/распознавание и подстановка текста).

### Отладка
- В случае ошибки авторизации проверьте корректность `API Key` и `Folder ID`.
- Для нестандартных форматов аудио используйте LPCM 16kHz mono.
