...
### Фаза 1. Подготовка
- [x] Утвердить данный ROADMAP
- [x] Создать интерфейсы в `domain` (`SpeechToTextService`, `SttConfig`, ошибки)
- [x] Определить параметры записи и стратегии форматов (oggopus по умолчанию)

### Фаза 2. Инфраструктура STT (Yandex)
- [x] Реализовать `YandexSpeechSttService` (авторизация Bearer/Api-Key, форматы oggopus/lpcm, WAV→PCM)
- [x] Обработка ошибок/таймаутов, пустых результатов
- [x] Логи (debug), трейсинг ключевых шагов
- [ ] Unit-тесты парсинга WAV и построения запросов

### Фаза 3. Запись аудио
- [x] Реализовать запись с микрофона (16kHz mono, огг/опус)
- [x] Состояния записи/отмены, очистка ресурсов
- [ ] Интеграционные тесты с моковым STT

### Фаза 4. UI-интеграция (плагин)
- [x] Кнопка микрофона в чат (справа под полем ввода)
- [x] Состояния UI: запись/стоп, распознавание, ошибка
- [x] Подстановка текста в поле ввода (без авто-отправки)
- [ ] E2E сценарий: «нажал → проговорил → стоп → текст в поле»

### Фаза 5. Документация и релиз
- [x] Обновить `docs/README.md` с инструкцией по конфигурации Yandex STT
- [x] Пример переменных и секретов
