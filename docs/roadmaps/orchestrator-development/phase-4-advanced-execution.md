...
- [ ] DAG корректно строит граф зависимостей
- [ ] Топологическая сортировка работает правильно
- [ ] Параллельное выполнение работает с ограничением потоков
- [ ] ExecutionContext передает данные между шагами
- [ ] Retry-логика с экспоненциальным backoff реализована
- [ ] Транзакционность и откаты работают корректно
- [ ] Обработка ошибок в параллельных задачах
- [ ] Unit и интеграционные тесты >80% покрытия
