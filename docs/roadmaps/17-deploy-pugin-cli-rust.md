...
- [ ] Инициализация Rust проекта с Cargo
- [ ] Настройка зависимостей (tokio, clap, serde, anyhow, reqwest)
- [ ] Создание модульной структуры директорий
- [ ] Реализация базового CLI интерфейса с Clap
- [ ] Настройка конфигурации через TOML + .env файлы
- [ ] Поддержка dotenv для загрузки переменных окружения
- [ ] Валидация наличия всех необходимых секретов при запуске
- [ ] Создание моделей данных для проекта, плагина, репозитория
- [ ] Базовая обработка ошибок с anyhow/thiserror

### Фаза 2: Ядро системы сборки
**Срок**: 2-3 дня
**Приоритет**: Высокий

- [ ] Реализация `builder.rs` - системы сборки плагинов
- [ ] Автодетекция проектов (Gradle/Maven)
- [ ] Параллельная сборка с индикацией прогресса
- [ ] Валидация артефактов (checksum, структура)
- [ ] Кэширование зависимостей
- [ ] Команда `deploy-pugin build` с опциями версии и профиля
...
- [ ] Реализация `yandexgpt.rs` - HTTP клиента для YandexGPT API
- [ ] Создание `agents.rs` - LLM агентов
- [ ] Разработка промптов в `prompts.rs`:
  - [ ] Changelog Agent промпт
  - [ ] Version Suggestion Agent промпт
  - [ ] Release Notes Agent промпт
- [ ] Асинхронные запросы к API с retry логикой
- [ ] Кэширование запросов для экономии токенов
- [ ] Обработка ошибок API с контекстуализацией
...
- [ ] Реализация `git/history.rs` - анализ git истории
- [ ] Реализация `git/tags.rs` - работа с тегами
- [ ] Анализ коммитов между версиями
- [ ] Детекция типа изменений (features, fixes, breaking)
- [ ] Интеграция с LLM агентами для анализа изменений
- [ ] Тесты git операций
...
- [ ] Команда `deploy-pugin ai changelog` с флагами (--since, --verbose)
- [ ] Команда `deploy-pugin ai suggest-version` (--analyze-commits)
- [ ] Команда `deploy-pugin ai release-notes` (--template)
- [ ] Мульти-модельная поддержка (OpenAI, Anthropic, локальные модели)
- [ ] Настройка разных агентов в конфигурации
- [ ] Валидация и фильтрация LLM ответов
- [ ] Тесты LLM команд
