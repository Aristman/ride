...
- [ ] Создать `MCPConnectionManager` сервис
...
- [ ] Добавить кэширование списка методов
- [ ] Добавить логирование событий подключения/отключения

### 5. UI - Страница настроек MCP
- [ ] Создать `MCPSettingsPanel` - панель настроек MCP
...
- [ ] Создать `MCPServerDialog` - диалог добавления/редактирования сервера
  - Поля: Name, Type (dropdown), Command, Args, Env, URL
  - Валидация полей
  - Кнопка "Test" для проверки подключения
- [ ] Добавить визуальные индикаторы статуса (зеленый/красный)
