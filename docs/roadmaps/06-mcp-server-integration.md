# Roadmap: Интеграция MCP Server в Ride Plugin

## Цель
Интегрировать MCP Server Rust в плагин Ride с автоматической установкой, запуском и подключением к Chat Agent через Yandex GPT Tools API.
...
- [ ] Обновить `ChatPanel`
  - [ ] Показывать индикатор "Executing tool..."
  - [ ] Отображать имя вызываемого tool
  - [ ] Показывать параметры (опционально)
  - [ ] Отображать результат tool call
- [ ] Добавить иконки для разных типов операций
  - [ ] 📄 для file operations
